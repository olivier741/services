...
public class ProcessBolt_Register extends BaseBasicBolt {
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
             logger.info("################################## START PROCESSING IN PROCESSBOLT_REGISTER ###########################");
             ...
             Product prod = SET_PRODUCT.get(process_req.getProduct_name());
             int state = 0;
             if (prod != null){
                 /*
             ...
             if(state == 1){
                    process_req.setNotification_code("REGISTER-PRODUCT-NOT-EXIST");
             ...
             if (state == 2){
                process_req.setNotification_code("REGISTER-PRODUCT-IS-EXPIRE-"+prod.getProduct_name());
                String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                logger.info("Emitte to SenderBolt : "+process_request);
             ...
             if (state == 3){
                process_req.setNotification_code("REGISTER-PRODUCT-WRONG-TIME-PERIOD-"+prod.getProduct_name());
                String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                logger.info("Emitte to SenderBolt : "+process_request);
             ...
             if(state == 0){
                 ...
                 */
                    ...
                    if (state1 == 1){
                        process_req.setNotification_code("REGISTER-PRODUCT-NOT-VALID-DAY-"+prod.getProduct_name());
                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                        logger.info("Emitte to SenderBolt : "+process_request);
                    ...
                    if (state1 == 2){
                        process_req.setNotification_code("REGISTER-PRODUCT-INVALID-FRAMETIME-"+prod.getProduct_name());
                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                        logger.info("Emitte to SenderBolt : "+process_request);
                   ...
                   if (state1==0){
                       ...
                       */
                       ...
                       if (state2==1){
                            process_req.setNotification_code("REGISTER-CUSTOMER-DENY-TABLE-"+prod.getProduct_name());
                            String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                            collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                            logger.info("Emitte to SenderBolt : "+process_request);
                       ...
                       if (state2==0){
                               ...
                               List<String> listrestrict_product = Arrays.asList(reg_time.split("-"));
                               List<Sub_Register> listRegister = ConnectionPool.getActiveSub_Reg(msisdn);
                               int state3 = 0;
                               if (listrestrict_product!=null && !listrestrict_product.isEmpty()){
                                    for (Sub_Register sub_reg: listRegister){
                                        if (listrestrict_product.contains(sub_reg.getProduct_name())){
                                            state3 = 1;
                                            break;
                                        }
                                    }
                            ...
                            if (state3==1){
                                        process_req.setNotification_code("REGISTER-PRODUCT-RESTRICTION-"+prod.getProduct_name());
                                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                                        logger.info("Emitte to SenderBolt : "+process_request);

                                        ConnectionPool.insertMo_his(msisdn,content, receiver, command, process_req.getParam_value(), receive_time, process_req.getAction_type(), "PRODUCT RESTRICTION TO : "+listrestrict_product +" OF "+prod.getProduct_name(), 0, "ProcessBolt");
                                        logger.info("insert into mo_his ");   
                             } 
                                    
                            if (state3==0){
                                int fee = prod.getReg_fee();
                                int state4 = 0;
                                if (fee>0){
                                    // request charging 
                                ...
                                if (state4==1){
                                        process_req.setNotification_code("REGISTER-PRODUCT-NOT-CHARGE-"+prod.getProduct_name());
                                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                                        logger.info("Emitte to SenderBolt : "+process_request);

                                        ConnectionPool.insertMo_his(msisdn,content, receiver, command, process_req.getParam_value(), receive_time, process_req.getAction_type(), "PRODUCT NOT CHARGE : "+prod.getProduct_name(), 0, "ProcessBolt");
                                        logger.info("insert into mo_his ");
                                }
                                
                                // regisger customer to service
                                if (state4==0){
                                    Timestamp paid_time = new Timestamp(System.currentTimeMillis());
                                    Timestamp expire_time = getExpiret_Time(prod.getValidity(),paid_time);
                                    
                                    ConnectionPool.insertRegister(msisdn, prod.getProduct_name(), prod.getReg_fee(), prod.getStart_time(), expire_time, prod.getEnd_time(),paid_time, prod.getValidity(), 1, prod.getIsextend(), prod.getRestrict_product());
                                    
                                    process_req.setNotification_code("REGISTER-PRODUCT-SUCCESS-"+prod.getProduct_name());
                                    String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                                    collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                                    logger.info("Emitte to SenderBolt : "+process_request);
                                    
                                    ConnectionPool.insertMo_his(msisdn,content, receiver, command, process_req.getParam_value(), receive_time, process_req.getAction_type(), "PRODUCT SUCCESS REGISTER : "+prod.getProduct_name(), prod.getReg_fee(), "ProcessBolt");
                                    logger.info("insert into mo_his ");
                                }
                            }
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
      ...
      private Timestamp getExpiret_Time(String validity, Timestamp paid_time){
