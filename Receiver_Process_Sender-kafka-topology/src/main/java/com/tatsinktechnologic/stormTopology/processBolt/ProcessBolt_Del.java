...
public class ProcessBolt_Del extends BaseBasicBolt {
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
             logger.info("################################## START PROCESSING IN PROCESSBOLT_DEL ###########################");
             ...
             if(state == 1){
                    process_req.setNotification_code("DELETE-PRODUCT-NOT-EXIST");
             ...
             if (state == 2){
                process_req.setNotification_code("DELETE-PRODUCT-IS-EXPIRE-"+prod.getProduct_name());
                String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                logger.info("Emitte to SenderBolt : "+process_request);
             ...
             if (state == 3){
                process_req.setNotification_code("DELETE-PRODUCT-WRONG-TIME-PERIOD-"+prod.getProduct_name());
                String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                logger.info("Emitte to SenderBolt : "+process_request);
             ...
             if(state == 0){
                 ...
                 */
                    ...
                    if (state1 == 1){
                        process_req.setNotification_code("DELETE-PRODUCT-NOT-VALID-DAY-"+prod.getProduct_name());
                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                        logger.info("Emitte to SenderBolt : "+process_request);
                    ...
                    if (state1 == 2){
                        process_req.setNotification_code("DELETE-PRODUCT-INVALID-FRAMETIME-"+prod.getProduct_name());
                        String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                        collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                        logger.info("Emitte to SenderBolt : "+process_request);
                   ...
                   if (state1==0){
                       ...
                       */
                       ...
                       if (state2==1){
                            process_req.setNotification_code("DELETE-CUSTOMER-DENY-TABLE-"+prod.getProduct_name());
                            String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                            collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                            logger.info("Emitte to SenderBolt : "+process_request);
                       ...
                       if (state2==0){
                             ...
                             for (Sub_Register sub_reg: listRegister){
                                 if (sub_reg.getProduct_name().equals(process_req.getCommand_code())){
                                     isregister = true;
                                     break;
                                 }
                             }
                             ...
                             if (isregister){
                                    ...
                                    ConnectionPool.updateRegister(msisdn, prod.getProduct_name());
                                    process_req.setNotification_code("DELETE-PRODUCT-SUCCESS-"+prod.getProduct_name());
                                    String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                                    collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                                    logger.info("Emitte to SenderBolt : "+process_request);
                                    
                                    ConnectionPool.insertMo_his(msisdn,content, receiver, command, process_req.getParam_value(), receive_time, process_req.getAction_type(), "SUCCESS DELETE : "+prod.getProduct_name(), prod.getReg_fee(), "ProcessBolt");
                                    logger.info("insert into mo_his "); 
                             }else{
                                    process_req.setNotification_code("DELETE-NOT-REGISTER-"+prod.getProduct_name());
                                    String process_request = ConverterJSON.convertProcess_rqToJson(process_req);
                                    collector.emit(KafkaTopology.SENDER_STREAM,new Values(process_request));
                                    logger.info("Emitte to SenderBolt : "+process_request);
                                    
                                    ConnectionPool.insertMo_his(msisdn,content, receiver, command, process_req.getParam_value(), receive_time, process_req.getAction_type(), "NOT REGISTER TO PRODUCT : "+prod.getProduct_name(), prod.getReg_fee(), "ProcessBolt");
                                    logger.info("insert into mo_his ");  
                             }
