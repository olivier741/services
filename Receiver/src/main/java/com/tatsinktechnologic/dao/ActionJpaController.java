...
public class ActionJpaController implements Serializable {
    ...
    public void edit(Action action) throws IllegalOrphanException, NonexistentEntityException, Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            ...
            for (Command listCommandOldCommand : listCommandOld) {
                if (!listCommandNew.contains(listCommandOldCommand)) {
                    ...
                    illegalOrphanMessages.add("You must retain Command " + listCommandOldCommand + " since its action field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
    ...
    public void destroy(Long id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            Set<Command> listCommandOrphanCheck = action.getListCommand();
            for (Command listCommandOrphanCheckCommand : listCommandOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Action (" + action + ") cannot be destroyed since the Command " + listCommandOrphanCheckCommand + " in its listCommand field has a non-nullable action field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
