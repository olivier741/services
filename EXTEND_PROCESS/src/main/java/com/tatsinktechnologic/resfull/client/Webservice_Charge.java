...
public class Webservice_Charge {
    ...
    public WS_Block_Response requestCharge_Product(String msisdn,String transaction_id,Product product){
        ...
        String fee_value ="0";
        if (promo!=null){
            long product_fee = product.getExtend_fee();
            long promotion_percentage = promo.getPercentage_ext();
            long promotion_fee = promo.getPromotion_ext_fee();
            ...
            fee_value = String.valueOf(product.getExtend_fee());
            param_description.setAlias(charge_aliase.getDescription());
