...
public class Process_Check implements Runnable{
    ...
    public Process_Check(EntityManagerFactory emf,BlockingQueue<Process_Request> check_queue,int sleep_duration) {
        this.emf = commonConfig.getEmf();
        this.check_queue=check_queue;
    ...
    public void run() {
        ...
        while (true){
            ...
            Process_Request process_mo = null;
             try{
                //consuming messages 
                process_mo = check_queue.take();
                logger.info("Get message in Check queue :"+ process_mo);
                logger.info("Check Queue size :"+ check_queue.size());
            } catch (InterruptedException e) {
