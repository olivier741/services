...
public class Process_Check implements Runnable{
    private static Logger logger = Logger.getLogger(Process_Check.class);
    ...
    private static BlockingQueue<Process_Request> chek_queue;


    public static void addMo_Queue(Process_Request process_req){
        try {
            chek_queue.put(process_req);
    ...
    public Process_Check(EntityManagerFactory emf,BlockingQueue<Process_Request> chek_queue,int sleep_duration) {
        this.emf = commonConfig.getEmf();
        this.chek_queue=chek_queue;
    ...
    public void run() {
        ...
        while (true){
            ...
            Process_Request process_mo = null;
             try{
                //consuming messages 
                process_mo = chek_queue.take();
                logger.info("Get message in Check queue :"+ process_mo);
                logger.info("Check Queue size :"+ chek_queue.size());
            } catch (InterruptedException e) {
