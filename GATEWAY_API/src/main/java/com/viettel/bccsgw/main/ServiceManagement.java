...
public class ServiceManagement
{
  ...
  public ServiceManagement(Logger logger, String appName)
  {
    this.logger = logger;
    this.appName = appName;
  }
  
  @WebMethod(operationName="gwOperation")
  @RequestWrapper(localName="gwOperationRequest")
  @ResponseWrapper(localName="gwOperationResponse")
  @WebResult(name="Result", partName="Result")
  public Output gwOperation(@WebParam(name="Input", partName="Input") Input parameters)
  {
    ...
    try
    {
      ...
      String userNameEnCrypt = parameters.getUsername();
      String passwordEnCrypt = parameters.getPassword();
      String password = "";
      try
      {
        userName = PassTranformer.decrypt(userNameEnCrypt);
        password = PassTranformer.decrypt(passwordEnCrypt);
      }
      catch (Exception e)
      {
        e.printStackTrace();
        this.logger.warn(errorCodeMap.get("6006"));
      }
      String seqRequest = getTransId();
      ArrayList<String> listOutput = new ArrayList();
      String wscode = parameters.getWscode();
      String ip = getIpClient();
      String responseCode = "";
      

      String inputValue = getInputFromParamList(parameters.getParam());
      String strRequest = formatRequest(userNameEnCrypt, passwordEnCrypt, wscode, parameters.getParam(), parameters.getRawData());
      if ((wscode != null) && (wscode.equals("BCCSReloadDB")))
      {
        ...
        output.setError("RELOAD_OK");
        output.setDescription("Reload database into memory success!");
      ...
      {
        releaseRequest(userName);
        saveLogDB(seqRequest, userName, wscode, strRequest, inputValue, ip, checkData, checkData.getError(), startTime, new Date(), null, System.currentTimeMillis() - begin, null, "0", true);
      ...
      {
        releaseRequest(userName);
        saveLogDB(seqRequest, userName, wscode, strRequest, inputValue, ip, validate, validate.getError(), startTime, new Date(), null, System.currentTimeMillis() - begin, null, "0", true);
      ...
      if (((Long)connectionManager.get(key)).longValue() >= maxConnection)
      {
        ...
        if (isReturn)
        {
          ...
          output.setDescription((String)errorCodeMap.get(errorCode));
      ...
      if (((Long)webserviceManager.get(wscode)).longValue() >= ws.getMaxConnection())
      {
        ...
        long waitingTimeout = Long.parseLong(ResourceBundleUtils.getResource("waitingTimeout"));
        long lStartTime = startTime.getTime() + waitingTimeout * 1000L;
        while (((Long)webserviceManager.get(wscode)).longValue() >= ws.getMaxConnection())
        {
          try
          {
            Thread.sleep(100L);
      ...
      if (XmlUtil.xmlCheck(resultRA))
      {
        ...
        try
        {
          ...
          output.setError(responseCode);
          output.setDescription((String)errorCodeMap.get(responseCode));
          output.setReturn(listOutput);
          output.setOriginal(resultRA);
          
          saveLogDB(seqRequest, userName, wscode, strRequest, inputValue, ip, output, responseCode, startTime, new Date(), Long.valueOf(timeCallWS), System.currentTimeMillis() - begin, null, resultCode, true);
        }
        catch (Exception ex)
        {
          responseCode = "6005";
          ...
          saveLogDB(seqRequest, userName, wscode, strRequest, inputValue, ip, output, responseCode, startTime, new Date(), Long.valueOf(timeCallWS), System.currentTimeMillis() - begin, ex, "0", true);
        }
      }
      else
      {
        ...
        output.setReturn(listOutput);
        output.setOriginal(resultRA);
        saveLogDB(seqRequest, userName, wscode, strRequest, inputValue, ip, output, responseCode, startTime, new Date(), Long.valueOf(timeCallWS), System.currentTimeMillis() - begin, null, "1", true);
      }
    }
    catch (Exception e)
    {
  ...
  private synchronized void releaseRequest(String userName)
  {
    ...
    {
      localLong1 = totalCurrentRequest;localLong2 = ServiceManagement.totalCurrentRequest = Long.valueOf(totalCurrentRequest.longValue() - 1L);
    }
    if (totalCurrentRequestForClient.longValue() > 0L)
    {
      localLong1 = totalCurrentRequestForClient;localLong2 = ServiceManagement.totalCurrentRequestForClient = Long.valueOf(totalCurrentRequestForClient.longValue() - 1L);
    }
  ...
  {
    Long localLong1 = totalCurrentRequest;Long localLong2 = ServiceManagement.totalCurrentRequest = Long.valueOf(totalCurrentRequest.longValue() + 1L);
  }
  ...
  private synchronized void increaseRequestForClient(RequestInfo reqInfo)
  {
    if (reqInfo != null)
    {
      Long localLong1 = totalCurrentRequestForClient;Long localLong2 = ServiceManagement.totalCurrentRequestForClient = Long.valueOf(totalCurrentRequestForClient.longValue() + 1L);
  ...
  protected void saveLogDB(String seqRequest, String userName, String wsCode, String request, String isdn, String ip, Output response, String responseCode, Date startDate, Date endDate, Long vasDuration, long duration, Exception exp, String resultCode, boolean hasAlert)
  {
    try
    {
      if (request != null) {
      ...
      if (response != null)
      {
        if ((response.getError() != null) && (!response.getError().equals(""))) {
          strMessage = strMessage + "<error>" + response.getError() + "</error>" + "#@n@#";
        }
        if ((response.getDescription() != null) && (!response.getDescription().equals(""))) {
          strMessage = strMessage + "<description>" + response.getDescription() + "</description>" + "#@n@#";
        }
        ArrayList<String> lstReturn = response.getReturn();
        if ((lstReturn != null) && (lstReturn.size() > 0)) {
          for (int i = 0; i < lstReturn.size(); i++)
          {
            strAlert = strAlert + (String)lstReturn.get(i);
            if (i != lstReturn.size() - 1) {
              strAlert = strAlert + "\n";
            }
          }
        }
      ...
      strResponse = strResponse + "<original>" + (response != null ? response.getOriginal() : "?") + "</original>";
