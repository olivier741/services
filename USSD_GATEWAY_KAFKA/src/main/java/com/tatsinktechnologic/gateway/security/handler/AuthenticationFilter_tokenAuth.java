...
public class AuthenticationFilter_tokenAuth implements ContainerRequestFilter{
    ...
    private static String IP_allow = null;
    
    static {
        IP_allow = communConf.getApp_conf().getIp_allow();
    }
    ...
    private void abortWithUnauthorized(ContainerRequestContext requestContext,Exception e) {
        ...
        requestContext.abortWith(Response.status(Response.Status.UNAUTHORIZED)
                                         .header(HttpHeaders.WWW_AUTHENTICATE, " realm=\"" + e.getMessage() + "\"")
                                         .build());
    }
     ...
     private boolean checkIP() { 
        ...
        List<String> listIP= Arrays.asList(ptn.split(IP_allow));
