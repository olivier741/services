    ...
    extends IoHandlerAdapter
  {
    ...
    public void sessionOpened(IoSession session)
    {
      ...
      else
      {
        ...
        UssdMessage randomInfoMsg = new UssdMessage(201);
        randomInfoMsg.setMsisdn(randomInfo);
    ...
    public void messageReceived(IoSession session, Object message)
    {
      ...
      if (Receiver.this.haveAlreadySession(session))
      {
        if ((message instanceof UssdMessage)) {
          try
          {
            UssdMessage msg = (UssdMessage)message;
            
            msg.setSendRecvTime(System.currentTimeMillis());
            if ((msg.getEncryptedUssdString() == null) && (Receiver.this.allMsgEncrypted) && (msg.getUssdString() != null))
            {
              Receiver.this.logger.warn("ussd string is NOT encrypted. Reject this message");
            }
            else
            {
              if (msg.getEncryptedUssdString() != null)
              {
                Receiver.this.logger.info("ussd string is encrypted. Decode it first");
                if (Receiver.this.decoder != null) {
                  try
                  {
                    msg.setUssdString(Receiver.this.decode(msg.getEncryptedUssdString()));
                  }
                  catch (Exception e)
                  {
          ...
          {
            Receiver.this.logger.error("@ussdappinf.recvqueue - Receive queue is full!");
          }
        }
      }
      else if ((message instanceof UssdMessage))
      {
        UssdMessage loginReq = (UssdMessage)message;
        if (loginReq.getType() == 100)
        {
          UssdMessage loginRsp = new UssdMessage(203);
          Receiver.this.logger.info("login message. check user/pass");
          try
          {
            byte[] clientHash = loginReq.getEncryptedUssdString();
